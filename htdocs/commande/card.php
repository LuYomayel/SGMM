		...
		if ($action == 'validate') {
			...
			if (isModEnabled('stock') && !empty($conf->global->STOCK_CALCULATE_ON_VALIDATE_ORDER) && $qualified_for_stock_change) {
				...
				$formquestion = array(
					...
					array('type' => 'other', 'name' => 'idwarehouse', 'label' => $langs->trans("SelectWarehouseForStockDecrease"), 'value' => $formproduct->selectWarehousesByIdPedido(GETPOST('idwarehouse', 'int') ? GETPOST('idwarehouse', 'int') : 'ifone', 'idwarehouse', $idPedido, '', 1, 0, 0, '', 0, $forcecombo))
				);
			}
			...
			if (empty($reshook)) {
				...
				if (empty($user->socid)) {
					if ($object->statut > Commande::STATUS_DRAFT || !empty($conf->global->COMMANDE_SENDBYEMAIL_FOR_ALL_STATUS)) {
						if ($usercansend) {
							// if the object status is draft then the button should say 'Pedir aprobacion'
